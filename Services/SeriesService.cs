...
namespace SeriesMovieTrailers.Services
{
    public class SeriesService : ISeriesService
    {
        ...
        private const string SeriesCachePrefix = "series:";


        public SeriesService(ITmdbService tmdb, ApplicationDbContext db, IDistributedCache cache)
        {
            _tmdb = tmdb; _db = db; _cache = cache;
        }


        public async Task<(IEnumerable<SeriesListItemDto> Items, int Total)> SearchAsync(string q, int page, int pageSize)
        {
            ...
            if (!string.IsNullOrEmpty(cached))
            {
                ...
                return (dto.SeriesItems, dto.Total);
            }


            var res = await _tmdb.SearchSeriesAsync(q, page);
            if (res == null) return (Enumerable.Empty<SeriesListItemDto>(), 0);


            var items = res.Results.Select(r => new SeriesListItemDto
            {
                Id = r.Id,
                Title = r.Title,
            ...
            var dtoOut = new SearchResultDto { Total = res.TotalResults, SeriesItems = items };
        ...
        public async Task<SeriesDetailDto?> GetByExternalIdAsync(int tmdbId)
        {
            var cacheKey = SeriesCachePrefix + tmdbId;
            var cached = await _cache.GetStringAsync(cacheKey);
            if (!string.IsNullOrEmpty(cached)) return JsonSerializer.Deserialize<SeriesDetailDto>(cached);


            var details = await _tmdb.GetSeriesDetailsAsync(tmdbId);
            ...
            var dto = new SeriesDetailDto
            {
                ...
                Trailers = trailers,
                noSeassons = details.noSeassons,
                noEpisodes = details.noEpisodes
            };
